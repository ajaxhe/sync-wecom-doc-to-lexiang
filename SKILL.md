...
> 🔴 **三条职责原则（skill 的最高纲领，review 时逐条对照）**：
> 1. **严格按照脚本参数和字段配置的要求，引导 Agent 完成 `config.json` 的配置**；
> 2. **通过脚本执行导入任务**（0 token 消耗——正文/图片/文件全由服务端抓，Agent 侧不碰内容）；
> 3. **总结导入任务状态：根据接口返回成功或失败，失败给出原因。不要去判断文档、文件是否为空——
>    只要接口返回导入成功，这篇企微文档或文件的导入就是成功的；接口返回失败，就照实报
>    `failed_items[].failed_code` + `failed_reason` 原文**。
>    skill 不得替服务端解释
>    「这个失败其实算成功」（规则详见 `references/pitfalls.md`「二、成败与失败口径」）。
...
7. 🔴 **必须显式下发 `conflict_strategy`（脚本已内置，默认 `skip`），绝不依赖服务端默认** ——
   不传该字段时，服务端默认行为是**替换目标目录内容**（不在本次提交集合内的既有条目被删，任务却报 `succeed`）。
   另：**`dry-run` 对该策略是盲的**（不传 / replace / skip / keep_both 四路 `dry_run_stats` 完全一致），
...
`create` / `dry-run` 末尾固定输出**三档**，数据**全部取自接口与既有扫描**，脚本不自行判断「文档内容有没有被改过、是否为空」：
...
🔴 **任务结束判定只看 `data.status`**（不参考任何其他字段），官方枚举 5 个值：
`page_processing`（页面导入进行中 x/n）/ `processing`（正在导入，初始化/解压 zip）为进行中；
`succeed`（成功）/ `failed`（失败）/ `canceled`（已取消）为终态。到终态立即汇总输出。
🔴 **失败文档与原因的唯一来源 = `failed_items[]`**：`entries[].status` 里的 failed / failed_reason
**不作为失败口径**（`entries[]` 仅作进度展示，不参与任何判定）。🛡 工程兜底：脚本已在回包出口把
`entries[]` 条目级错误说明**统一过滤**
...
**失败时**才展开，且只给三样：`task_id`、终态、`failed_items[].failed_code` + `failed_reason` **原文**；
若失败与**授权 / 凭证**有关，附一句「导入的前提是乐享侧授权配置有效」+ 文档链接，其余排查一律不展开。
🔴 失败原因**照接口原文讲**，不要替服务端解释（如「这个失败码其实文件已导入」）——接口判失败就是失败。
...
**退出码**：`0` 成功（含「已有进行中任务 → 静默退出」供定时任务复用）；`1` 运行期失败（鉴权失效 / **任务 `failed` —— 含服务端判「非法的 'file_id'」等形态问题** / 任务 `canceled`（已取消）/ 凭证缺失 / 目录扫描失败 / 网络异常 / 轮询超时）；`2` 用法错误。

**任务结束判定只看 `data.status`**（唯一依据，不看 entries 等其他字段），到终态立即汇总输出。
状态枚举：`page_processing` / `processing` = 进行中（继续轮询）；`succeed` / `failed` / `canceled` = 终态。
轮询预算（次数 × 间隔）用尽仍未到终态 → `exit 1` 并提示稍后用 `status` 查询，**不要无限轮询**。
...
| ② 要同步的**企微资产**标识 | 在线文档 / 智能文档 / 表格：从企微界面复制**完整 URL**（含 `?scode=`）；微盘：复制**分享链接**（`https://drive.weixin.qq.com/s?k=…`，含 `?k=`） | 一条一个，**原样粘贴不要手改**（不要换成 docid / file_id）。**doc 类型链接（`https://doc.weixin.qq.com/doc…`）还要记下文档名称填 `name`**（见下节） |
...
### 🔴 doc 类型链接：必须补齐文档名称（`name` 字段）

`https://doc.weixin.qq.com/doc…` 开通的文档链接（doc 类型在线文档）**必须显式填 `name`**——
企微后端接口**无法通过文档 URL 获取文档名称**，不填 `name` 就导入，该文档在乐享里会变成
**「未命名文档」**。名称获取优先级（由高到低）：
...
```json
{ "id": "https://doc.weixin.qq.com/doc/w3_…?scode=…", "name": "《文档标题》" }
```

⚠️ 这是**临时逻辑**：已向企微提需求，待其后端接口支持按 URL 反查名称后**删除本节**（`name` 回归纯选填）。
...
#    doc 类型链接（doc.weixin.qq.com/doc…）必须同时填 "name"（文档名称，见上节），否则导入后变「未命名文档」
...
| 用户说 | 只改哪里 | 改完必须跑 |
|---|---|---|
| 「再加几个文档」 | `source.candidates` **末尾追加**对象（doc 类型链接记得同时补 `name`） | `dry-run` |
...
> `files[]` 元素有 `id`（必填，链接 URL）/ `name`（选填，文档名称，接口设计如此）/ `include_subpages`。**服务端不校验未知字段**（多余字段被静默忽略，不会报错、也不会生效）。
...
- 以下两个**可省略**（省略即取默认值）：
  - `name`（默认 `""`）：**文档名称**。接口设计：`id` = 链接 URL（身份），`name` = 文档名称，脚本随请求体 `files[].name` 一并提交。
...
**脚本构造的请求体**（不含本地字段；`files[].name` 为文档名称，选填，有值才传）：

```json
{ "space_id": "<target.space_id>", "type": "wecombot",
  "parent_entry_id": "<target.parent_entry_id>",
  "files": [{ "id": "<实际提交的 id 字符串>", "name": "<文档名称，选填，有值才传>", "include_subpages": true }],
  "dry_run": false,
...
- 配置位置：`source.conflict_strategy`；临时覆盖：`create --strategy <值>`。脚本一律**显式下发**该字段，
  绝不依赖服务端默认（缘由见红线 7 与 `references/pitfalls.md` 3.4）。
...
> 不用 Agent 操心；只在排查「为什么重复了」时才读 `references/pitfalls.md`「一、红线」与 `references/import-api.md` §4。
>
> 脚本输出的完整样例（`候选提交保真清单` / `dry_run_stats` / 同源 WARN 行）与**实现约束**，
> 见 `references/pitfalls.md`「五、输出样例与实现约束」。
...
| 候选写了微盘分享链接 | **正常形态，直接放行**（服务端已接受，原样提交即可）。**不要**换成 `file_id` —— 那是多余动作，且两者互不相等会造出重复条目 |
...
| 文件 | 内容 | 什么时候读 |
|---|---|---|
| `references/import-api.md` | **接口参数全表**（4 必填 + 2 可选 + `files[]` 元素字段 + 校验行为 + 非法值探测方法）、**MCP 调用路径（按可见 tools 选路）**、去重语义、`dry_run_stats`、异步时序、四条实测口径（`err_message` / `failed_items` / 部分失败 / 计数为 0 不回字段）、**⭐ 报告能取到什么（哪些维度接口没有）**、`entry_list_children` 分页 | 要改接口调用时 / 要动报告字段时 |
...
| `references/pitfalls.md` | 红线 + **成败与失败口径** + 已知限制与行为规则（索引只扫一层 / conflict_strategy / 调用路径 / 诊断守则）+ 接口侧踩坑备忘 + **输出样例与实现约束** | 动 `norm()` 或索引逻辑前**必读**；排查「为什么重复了 / 为什么没匹配上 / 为什么任务 failed」 |
