...
`create` / `dry-run` 末尾固定输出**四档**，数据**全部取自接口与既有扫描**，脚本不自行判断「文档内容有没有被改过」：

| 档 | 来源 |
|---|---|
| ✅ 新增（`add_num`） | 候选归一后未命中目端既有 `source.href.id` |
| ♻️ 已存在·复用（`special_num`） | 命中既有条目（按既有原始字符串提交 → 幂等复用），报告里带**目端条目名 + 目端链接** |
| ⚠️ 已导入·内容提示 | `failed_items[]`（或 `entries[].status`）里命中 `BENIGN_FAILED_CODES` 的项 —— **文件已导入成功**，仅服务端内容处理为空/受限，**不计失败**（2026-09-22 用户裁决） |
| ❌ 失败 | 接口 `failed_items[]` 的 `failed_code` / `failed_reason`（剔除内容提示类） |

**内容提示类失败码**（脚本常量 `BENIGN_FAILED_CODES`，现含 `video_content_empty`）：音频/视频等媒体文件
已成功导入乐享，只是内容转写/提取为空 —— 这是服务端内容处理层的事，**不是导入 skill 该管的**，
汇报时归入成功口径（如「✅ 成功 7 / 其中 1 条内容提示（video_content_empty）」），退出码按 0。
遇到**新的**同类失败码（文件实际已导入、仅内容处理异常）：先核实乐享端条目确实存在，再把它加进
`BENIGN_FAILED_CODES` 并同步本文档与 `references/import-api.md`。
...
内容提示类（`video_content_empty` 等）**不计入失败汇报**——文件已导入成功，一句话带过即可（如「#5 mp3 已导入，音频内容为空」）。
...
**退出码**：`0` 成功（含「已有进行中任务 → 静默退出」供定时任务复用；含任务 `failed` 但未成功项**全部**为内容提示类失败码 —— 文件均已导入成功）；`1` 运行期失败（鉴权失效 / **任务 `failed` 且存在真失败项 —— 含服务端判「非法的 'file_id'」等形态问题** / 任务到终态但条目排空超时 / 凭证缺失 / 目录扫描失败 / 网络异常 / 轮询超时）；`2` 用法错误。

**任务终态 ≠ 条目全部完成**（2026-09-22 用户反馈）：服务端会把仍在后台转存的大文件条目留在
