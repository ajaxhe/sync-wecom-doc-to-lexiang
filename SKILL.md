...
**退出码**：`0` 成功（含「已有进行中任务 → 静默退出」供定时任务复用）；`1` 运行期失败（鉴权失效 / **任务 `failed` —— 含服务端判「非法的 'file_id'」等形态问题** / 任务到终态但条目排空超时 / 凭证缺失 / 目录扫描失败 / 网络异常 / 轮询超时）；`2` 用法错误。

**任务终态 ≠ 条目全部完成**：服务端会把仍在后台转存的大文件条目留在
`entries[].status=processing`，任务却已先到终态。脚本行为：到终态后**继续轮询等条目排空**（同一轮询预算内），
排空后才输出汇总报告；预算用尽仍有条目处理中 → `exit 1` 并打印**给 Agent 的显式指令**：
