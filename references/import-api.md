...
> 事实源：`企业微信知识库导入-Skill需求摘要.md` + 本 skill 的实跑验证。
...
1. **选择性拒绝 ≠ 凭证问题**：`whoami` 直调成功即证明 token 有效；「只拒业务工具、同一 token 下包装就通」
   这种选择性拒绝，机制上不可能由凭证问题产生。
2. **「目录里可发现」≠「`tools/list` 里可见」**：`list_tool_categories` 能看到全部业务工具目录
   （`knowledge.entry` 13 个 / `knowledge.block` 21 个 …），但 `tools/list` 可能只回少量元工具——
   前者是注册表，后者是本次下发的可见集；服务端开关与本地客户端会话可能不同步（要放回 + 重启才刷新）。
3. 关于 `import_*`：`get_tool_schema` 对它们回 `tool not found`，而对 `entry_list_children`
...
**不传该字段 = `replace` 语义**：「本次提交集合」等于「保留下来的集合」；**不在集合内的既有条目会被删掉**
（命中本轮候选的条目保留原 `entry_id`，其余删除），任务自身报 `succeed`、`add_num=0`，**全程无任何删除提示**。
所以 `replace`（以及**不传该字段时的默认行为**）只能在**专用导入目录**上使用。

**三条必须记住的边界**：

1. **`dry_run` 对冲突策略是盲的**：不传 / `replace` / `skip` / `keep_both` 四路的 `dry_run_stats` **完全一致**。
   → **`dry-run` 结果不能作为「真跑不会动目的端」的依据**。
2. **回包不携带 `conflict_strategy`**：`import_describe_task` 的 `data` 里没有该字段，**事后无法追溯**当时用的是哪个策略，
   所以必须在下发时显式指定并在本地留痕（脚本会打印）。
3. **去重索引有延迟**：新建条目要过几分钟才进入服务端去重索引 → 不要用短时间内的两次 dry-run 差异下结论。
...
🔴 **无任何例外**：出现在 `failed_items[]` 里就是失败，照实报
`failed_code` / `failed_reason` 原文——**不判断**内容是否为空、不解释「其实已导入」、
不给任何失败码开白名单（详见 `pitfalls.md`「二、成败与失败口径」）。
...
### ⭐ 报告能取到什么（`render_report` 三档字段的全部来源）
...
- **条目级操作类型**：回包里既无 `entries[]`（多次真跑均未回），也无 `operation / created / updated / skipped` 之类字段。
...
→ 因此**同一份微盘资产只固化一种写法**：**分享链接原样提交**即可（服务端直接接受，
见 `pitfalls.md` 3.2）；老配置里的 `file_id` 可继续用，但**不要与链接混用**。
...
`import_*` 既不在 `tools/list`、`get_tool_schema` 也查不到 → **官方 schema 拿不到**，只能用「非法值探针」反解（全部 `dry_run=true`，不写库）：
...
- 因此本文参数表的结论是「**已确证的**」，不是「穷举的」。另用 60+ 个常见候选字段名
