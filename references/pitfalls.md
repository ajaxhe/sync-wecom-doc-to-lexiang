...
🔴 **这条判据还扛着一条设计要求（红线 9 · 免升级）**：接口将来放开通**新的文档类型**时，
本 skill **必须不改一行**就能导入 —— 因为**用户更新 skill 是不确定的行为**。
任何「按文档类型分叉」的硬编码（白名单 / 黑名单 / 形态禁入表 / 类型→参数映射）都会把新类型**卡在旧版本上**。
⇒ 判断权全部交回接口。验证方法见 `wecom-sources.md`「接口新增文档类型时：免升级设计」。
...
🔴 **无例外**：`failed` 就是 `exit 1`，不存在「失败码白名单 / 其实算成功」的分档（见 §2.11 变迁记录）。
...
解释失败码，就得维护一张「哪些失败其实算成功」的白名单——名单会过期、会漏判、会让
`exit code` 与接口结论不一致。上午版本里「分档后 exit 0」直接把「接口说 failed」翻成了
「脚本说成功」，正是 §2.4 判据禁止的事：**结论必须是接口刚刚告诉我的，不是我推断的**。

**现行口径（不可违反）**：
- 成败与原因**逐字来自接口回包**：`status` / `failed_items[].failed_code` + `failed_reason`；
- Agent 汇报失败时**照原文讲**，不替服务端解释、不建议「换个写法再试」；
- 判断「这条失败到底影响不影响使用」是**用户/服务端的事**，不是本 skill 的事。

**判据（想给任何失败码开例外前先问）**：这是不是在替服务端重新解释它的回包？是 → 不许做。
若服务端将来想让 `video_content_empty` 不算失败，**让接口侧不把它放进 `failed_items[]`** ——
那是唯一正确的修改位置。
