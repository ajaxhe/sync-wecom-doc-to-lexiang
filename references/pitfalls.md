...
> 澄清：微盘分享链接**不需要任何改写** —— 它本身就是服务端接受的形态（**2026-09-21 复验翻转**，见 §2.2）。
...
**不需要任何换算动作**。`REJECTED_KINDS` 已置空，脚本原样提交分享链接。

实测结果（候选 #3 用换来的 `file_id` 提交）：`MATCHED→复用既有`，
服务端 `special_num=3 / add_num=0`，**目标目录零新增**。

> ⚠️ **脚本现行行为**：`REJECTED_KINDS` 为**空集** → 分享链接**不再被拦截**，`build_plan()` 原样提交。
> 历史上「遇到分享链接就 `exit 1` + 打印换法指引」的那段逻辑**已随结论翻转删除**（见下方变迁表）。
...
>    `REJECTED_KINDS` 置空、脚本原样提交，**候选只填源端链接原文**。
...
> 📌 **同一条错误文案还有另一个来源**：`…/sheet/` URL（§2.4）—— 那是**当前仍成立**的拒收，
> 报错文案与微盘分享链接当年的报错**逐字相同**，别把两件事混起来。
...
| 候选 = 微盘分享链接 → `dry-run` / `create`（2026-09-21 上午） | 经 `REJECTED_KINDS` **直接拒收**：`exit 1`、未发出导入请求 |
| 候选 = 微盘分享链接 → `dry-run` / `create`（**2026-09-21 下午起，现行**） | **原样提交**（`REJECTED_KINDS` 已置空）：`MATCHED→复用既有`（若该链接身份已建过条目）或 `NEW→原样提交`；服务端**已接受**该形态。实测单条候选 `succeed 1/1`，条目 `href.id` = 链接原文 |
...
2. 候选若落在 `REJECTED_KINDS`，必须**直接拒收并打印换法指引**（`exit 1`、不发导入请求）——
   绝不猜测替换、绝不降级成原样提交。
   ⚠️ **该集合当前为空集**（2026-09-21）：微盘分享链接**不再属于拒收形态**（服务端已直接接受），
   **不要**按旧文案把它加回去。
