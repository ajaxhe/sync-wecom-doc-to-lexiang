...
`candidates[]` 里除了 `id` 还有可选的 `name`，填**文档名称**（如清单文档里链接的锚文本 / 文档标题）。
**接口设计：`id` = 链接 URL（身份），`name` = 文档名称**，脚本会把它放进请求体 `files[].name` 一并提交。

🔴 **doc 类型链接（`https://doc.weixin.qq.com/doc…`）必须显式填 `name`**：企微后端接口
**无法通过文档 URL 获取该类文档名称**，不填 `name` 导入后该文档会变成**「未命名文档」**。
名称获取优先级（由高到低）：

1. **待导入文档列表**（用户给的清单 / 对话中出现过的标题 / 链接锚文本）——首选；
2. 实在拿不到 → **企微连接器**（文档列表 / 搜索）按 URL 对应关系取回名称。

⚠️ **临时逻辑**：已向企微提需求，待其后端接口支持按 URL 反查名称后**删除本节**（`name` 回归纯选填）。
🔴 脚本提交的身份永远是 `id` 原文（`name` 不做身份覆盖）；
...
#    （id 必填；doc 类型链接必须同时填 name——名称就取清单里的锚文本/标题，
#      企微接口无法通过 URL 反查该类文档名称，漏填导入后会变「未命名文档」）
...
```jsonc
{ "candidates": [
    { "id": "https://doc.weixin.qq.com/doc/w3_…?scode=…", "name": "文档名称（doc 类型必填，否则导入后变「未命名文档」）" }
] }
```
