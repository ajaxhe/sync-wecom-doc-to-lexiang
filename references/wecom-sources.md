...
🔴 **doc 类型链接（`https://doc.weixin.qq.com/doc…`）必须显式填 `key`**：企微后端接口
**无法通过文档 URL 获取该类文档名称**，不填 `key` 导入后该文档会变成**「未命名文档」**。
名称获取优先级（由高到低）：

1. **待导入文档列表**（用户给的清单 / 对话中出现过的标题 / 链接锚文本）——首选；
2. 实在拿不到 → **企微连接器**（文档列表 / 搜索）按 URL 对应关系取回名称。

⚠️ **临时逻辑**：已向企微提需求，待其后端接口支持按 URL 反查名称后**删除本节**（`key` 回归纯选填）。
...
#    （id 必填；doc 类型链接必须同时填 key——名称就取清单里的锚文本/标题，
...
```jsonc
{ "candidates": [
    { "id": "https://doc.weixin.qq.com/doc/w3_…?scode=…", "key": "文档名称（doc 类型必填，否则导入后变「未命名文档」）" }
] }
```
