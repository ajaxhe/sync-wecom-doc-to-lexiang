...
🔴 2026-09-21 用户裁决：原「身份覆盖 / 逃生口」语义已**删除** —— 脚本提交的身份永远是 `id` 原文；
...
## 接口新增文档类型时：**免升级设计**（2026-09-21 用户要求）

**要求原话**（用户）：

> 「因为未来接口会支持更多的文档类型，不然到时候还要单独更新 skill，而对用户来说，更新 skill 是**不确定的行为**。
> 我希望未来接口能力支持，skill 不需要做升级，就可以马上支持新的文档类型导入。」
...
**唯一曾经会「卡住新类型」的东西就是类型白名单 / 黑名单 / 形态分档**（`REJECTED_KINDS` 等）。
它们已于 2026-09-21 全部删除（见上一节）—— 删它们的**第二个理由**正是这条免升级要求。
...
2026-09-21 实测（4 条假想形态：`…/mind/`、`…/flowchart/`、`brandnew.example.com/…`、纯字符串）：
