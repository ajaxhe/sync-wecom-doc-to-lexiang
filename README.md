...
✅ **微盘分享链接服务端已直接接受**（2026-09-21 复验：单条候选 `succeed 1/1`，条目 `source.href.id` = 链接原文）。
**不需要**换算成 `file_id`；`file_id` 只是兼容老配置，且与链接**互不相等**（混用会出现两条）。

> 📦 历史：2026-09-20 该形态曾被服务端拒收（`failed_reason`「非法的 `file_id`」），脚本当时会中止并提示换算。
> 该结论已于 2026-09-21 复验推翻，相应拦截逻辑已删除（详见 `references/pitfalls.md` §2.2）。
