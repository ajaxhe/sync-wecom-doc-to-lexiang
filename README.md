...
`1` 运行期失败（鉴权失效 / 任务 `failed` / 任务到终态但仍有条目处理中（排空超时，
会打印「等全部处理完再跑 status 汇总」的 Agent 指令）/ 凭证缺失 / 目录扫描失败 / 网络异常 / 轮询超时）；
`2` 用法错误。

> 🔴 **成败口径**：接口返回导入成功就是成功、返回失败就是失败（照实报 `failed_code` / `failed_reason` 原文）。
> 脚本不判断文档/文件内容是否为空、不解释任何「失败其实算成功」——本 skill 只做三件事：
> 引导配置 → 执行脚本 → 告知结果。
...
> ⚠️ **一次实测 ≠ 长期事实**：能不能导入由服务端**当前**行为决定——这也是本 skill
> 不写任何支持性结论的原因（详见 `references/pitfalls.md` 3.2、3.3）。
